...
class BertSelfAttention(nn.Module):
    def __init__(self, bert_config, bert_self_attention, score_util):
        ...
        self.attention_head_size = int(
            bert_config.hidden_size / bert_config.num_attention_heads
        )
        ...
        if self.score_util == "pp":
            self.score_lambda = nn.Parameter(
                torch.randn(1, bert_config.num_attention_heads, 1, 1)
            )  # (1, nh, 1, 1)
            # (1) ones
...
class BertEncoder(nn.Module):
    def __init__(self, bert_config, pretrained_model_opts, score_util, n_layers=12):
        super().__init__()
